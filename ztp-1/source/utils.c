...
void PrintSettings(){
    ...
    Ql_GetLocalTime(&loct);
     ...
     OUTER("\r\nAPN=%s IP=%d.%d.%d.%d TCP=%d USER=%s PASS=%s DEBUG=%s NUM=%d TSEND=%d DATE=%.2d.%.2d.%.2d TIME=%.2d:%.2d:%.2d DEBOUNCE=%d LocalIp=%d.%d.%d.%d\r\n\r\n ***** SIM:%d GSM:%d GPRS:%d PINS=%s SIGNAL:%ddBm ******\r\n",
                      __settings.APN,__settings.IP[0],__settings.IP[1],__settings.IP[2],__settings.IP[3],
                      __settings.TCP,__settings.User,__settings.Pass,__settings.Debug,__settings.Num,__settings.TSend,
                     loct.day,loct.month,loct.year,loct.hour,loct.minute,loct.second,__settings.Deboung,
                     __local_ip_addr[0],__local_ip_addr[1],__local_ip_addr[2],__local_ip_addr[3],simcard,creg,cgreg,&pp[0],rssi);
}
