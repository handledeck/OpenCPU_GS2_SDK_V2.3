...
typedef enum
{
    QL_SAM_PCM16   = 0,  // pcm 16 bit   , and the sample is 8K.
    QL_SAM_PCM8,         // pcm 8 bit  , 8 bit unsigned and the sample is 8K.
    QL_SAM_ALAW,       // pcm 8 bit  , 8 bit a-law  and the sample is 8K.
    QL_SAM_MULAW,   // pcm 8 bit  , 8 bit u-law  and the sample is 8K.
    QL_SAM_INVALID
}QlStreamFromat;
...
typedef void (*OCPU_REC_CB)(u8 * pData, u32 dataLen);// callback for StreamRecord , and pData is invalid
...
*****************************************************************/
s32 Ql_WDTMF(u8 ul_volume, u8 dl_volume, ascii *str ,u16 len);
...
/*****************************************************************
* Function:     Ql_StreamRecordSetBuffer
* 
* Description:
*                 The function set a buffer to stroe  the record PCM data.
*                 When the PCM data fill the buffer and reached more than half of the buffer,  then callbcak function 'cb_rec' will be invoked
*                 
* Parameters:
*                pBuffer: the buffer address. you can use Ql_GetMemory to allocate this memory,or define a large array. 
*					the size of buffer at least 6400
*                
*                bufferSzie: the size of the buffer. 
*
*		NOTE: The buffer you can't write or read it , it is controlled by the driver task. 		
*
* Return:              
*               QL_RET_OK, suceess
*               QL_RET_ERR_PARAM, indicates error.
*****************************************************************/
s32 Ql_StreamRecordSetBuffer(u8 *pBuffer, u32 bufferSzie);

/*****************************************************************
* Function:     Ql_StreamRecordStart
* 
* Description:
*                 The function start to stream record.
*			Before record you need use Ql_VoiceCallChangePath to chose the MIC for record.
*                 
* Parameters:
*                format: one value of the QlStreamFromat,  please refer this enum.
*                
*                cb_rec: the callback function, when the pcm data is  reached the threshold.
*                            this callback is called by a messge form media task.
*
*		NOTE: 
*		    If stream fromat is 16 bit, pre 20ms PCM data is 320 bytes. 
*				so you must give a large buffer to store the PCM data, in case there are too many message in app task  case the system crash.
*               If stream fromat is 8 bit, pre 20ms PCM data is 160 bytes.
*
* Return:              
*               QL_RET_OK, suceess
*               QL_RET_ERR_PARAM, indicates error.
*
*****************************************************************/
s32 Ql_StreamRecordStart(QlStreamFromat format, OCPU_REC_CB cb_rec);

/*****************************************************************
* Function:     Ql_StreamRecordReadBuffer
* 
* Description:
*                 The function is use to read record data, when the 'cb_rec'  callback is called.
*                 
* Parameters:
*                pData: a buffer use to stroed the data which read out from the PCM data buffer.
*                
*                readLen: the data want to read.
*
* Return:              
*           the size of the data read out.
*****************************************************************/
s32 Ql_StreamRecordReadBuffer(u8 * pData, u32 readLen);

/*****************************************************************
* Function:     Ql_StreamRecordStop
* 
* Description:
*                 The function stop sream record.
*                 
* Return:              
*           QL_RET_OK, suceess
*****************************************************************/
s32 Ql_StreamRecordStop(void);
