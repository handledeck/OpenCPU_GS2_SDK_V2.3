...
void CheckTypePacket(unsigned char* packet, u16 len){
	...
	case ConfirmReceipt:
	{
        ...
        if (pnum==__numPacket) {
            __countRepeat=0;
             OUTD("Recive confirm packet number:%d.", pnum); 
             ConfirmMsg(); 
              ...
              __waitConfirm=0;
               ...
               OUTD("!!!!!!!Send number packet %d and recive packet:%d have different number",__numPacket,pnum);
                  ...
                  if (__countRepeat>2) {
                      Ql_Sleep(200);
                      OUTD("!!!!!!!!Count repeat differnt packet more than 3.Alert system reboot",NULL);  
                      Ql_Reset(0);
                  }
         }      
		break;
	...
	case Text:{
		packet[len+1]='\0';
        OUTD("Recive text packet :%s.",++packet);
...
void ParseTextCommand(char* command,unsigned char len){
		...
		if (confing){
            ...
            OUTD("Set local time result:%d-%d-%d %d:%d:%d result:%d",year,month,day,hour,minute,second,loctime);
		}
	}
