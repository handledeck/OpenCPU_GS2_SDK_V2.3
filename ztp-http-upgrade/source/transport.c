        ...
        if(index >= 0)
         {
            ...
            __client[index].protocol=NONE;
            __client[index].pRecvCurrentPos=0;
         }
        Ql_SocketClose(sock);
    }
		...
		if (sock==__est_connector_socket) {
            ...
            if (ret>0) {
                    ...
                    if (__client[index].protocol==HTTP) {
                        HttpHeader header;
                        if(parse_header(&__client[index],&header))
                        {
                            if (header.method==GET) {
                               ...
                               //Ql_SocketSend(sock,(u8*)__s_buf,Ql_strlen(__s_buf));
                               //s32 ss=__client[index].send_handle(sock,__s_buf);
                               u16 pos=0;
                               u16 size=256;
                               s32 ind;
                               while (TRUE) {
                                    OUTD("SEND:%d POS:%d SIZE:%d",ind,pos,size); 
                                   if (pos+size>Ql_strlen(__s_buf)){
                                       size=Ql_strlen(__s_buf)-pos;
                                   }
                                   index=Ql_SocketSend(sock, (u8*)&__s_buf[pos], size);
                                    Ql_Sleep(100);
                                   pos+=size;
                                   if (Ql_strlen(__s_buf)==pos){
                                       break;
                                   }
                    ...
                    /*if(read_request(__client[index].recvBuffer))
                        __client[index].send_handle(sock,__s_buf);
                    else
                    {
                        OUTD("send to uart :%d byets",ret);
